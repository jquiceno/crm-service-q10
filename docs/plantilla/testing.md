...
**Ojo con las rutas:** no hay prefijo de **versión**, pero sí un **prefijo de servicio** (`RoutePrefix`) que `GlobalRoutePrefixConvention` antepone a **todas** las rutas de controller. Las rutas de controller salen de su `[Route(...)]` relativo, con una vuelta de tuerca: `Program.cs` registra un `RouteTokenTransformerConvention` con `KebabCaseParameterTransformer`, así que los **tokens** `[controller]`/`[action]` se pasan a kebab-case antes de volverse URL. Combinado con el prefijo, un `ProductCatalogController` con `[Route("[controller]")]` responde en `/{RoutePrefix}/product-catalog`. Los segmentos literales quedan tal cual. Las de health no son de controller: se registran con `MapHealthChecks` en `Program.cs`, que las prefija explícitamente (`/{RoutePrefix}/health/live`, `/{RoutePrefix}/health/ready`).

Todas las rutas viven bajo el prefijo de servicio (`RoutePrefix`, ej. `/service-template`): `GlobalRoutePrefixConvention` lo antepone a los controllers y `Program.cs` lo antepone a los endpoints minimal-API (health, OpenAPI). No se usa `UsePathBase`; el ALB reenvía el path completo. Como el prefijo se define en `appsettings.json`, aplica también en los tests in-process, que llaman a la ruta **con** prefijo. Ver `ServiceInfoEndpointsTests.cs` (`/service-template/info`) y `HealthProbesTests.cs` (`/service-template/health/live`).
