...
- **Ejecución parcial (2026-08-21) — puntos 1 y 2 entregados, punto 3 bloqueado por GAP-B.** Quedan hechos el caso de uso, los DTOs, el mapping, el validador, la acción de controller y el registro DI, con 15 pruebas unitarias nuevas. **La caché L2 no se implementó:** ver `⚠️ GAP-B` en §9.2. Dos desviaciones de la lista de archivos de este paso, ambas por convención real del repositorio:
...
**⚠️ GAP-B — `BLOQUEANTE` para la caché L2 · abierto 2026-08-21 · afecta F3 (punto 3), F5, F6 y F7 (invalidación post-commit)**
...
Hasta que se decida, el listado consulta la base en cada petición: **correcto pero sin la caché de D12**, que es el defecto D-12 que este plan quería corregir. `CreateBusinessStatusUseCase` (F5, ya en la rama) tampoco invalida L2 por el mismo motivo.

**Ningún otro GAP `BLOQUEANTE` está abierto. Ningún paso de §8 nace `blocked`.** (`GAP-B`, abierto el 2026-08-21 al ejecutar F5, es `NO BLOQUEANTE`: bloquea una línea de F5/F6/F7, no un paso completo.)

| GAP | Estado | Resolución del 2026-08-14 |
|---|---|---|
| **GAP-B (código de tenant para la llave L2)** | **Abierto — `NO BLOQUEANTE`** · detectado 2026-08-21 al ejecutar F5, reincidente en F6 | §7.2 exige `CacheKey.For("businessstatus").Tenant(tenantCode)`, pero **ninguna capa por debajo de HTTP conoce `tenantCode`**: `TenantMiddleware` lee `X-Entity-Code`, resuelve el `TenantInfo` y propaga **solo** el connection string vía `ITenantConnectionInitializer` → `TenantContext`; `TenantInfo.EntityCode` no sale del middleware, y no hay `IHttpContextAccessor` registrado. **Decide quien implemente F3**, que es la slice que construye la llave: lo natural es un accesor de solo lectura análogo a `ITenantConnectionInitializer` (p. ej. `ITenantCodeAccessor` en `Api/Session/`, poblado por el middleware, expuesto como puerto a Application). Al cerrarlo hay que agregar la invalidación L2 a F5 y F6 (ambas ya ejecutadas sin ella, R-10) y a F7 |
