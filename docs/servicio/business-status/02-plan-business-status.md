...
> **✅ `DESVIACIÓN-2` — RATIFICADA por el Tech Lead el 2026-08-28.** Ambos cambios de superficie compartida quedan aprobados: `ITenantCodeProvider` (GAP-B / B-1) y `PagedPayload<T>` público. El veredicto los reconoce como correctos y alineados con el contrato documentado de `Shared`.
>
> **Observación de gobierno, para la próxima vez:** por §0 (≤ 400 líneas de producción o ≤ 10 archivos de `src/` por PR, y «un PR no mezcla dominio, infraestructura y API») lo limpio habría sido entregar el cableado del código de tenant —B-1: el puerto de `Shared` más middleware y sesión— **como PR propio**, reutilizable por F5/F6/F7, dejando F3 reducida a la slice de listado. Se acepta tal cual esta vez porque quedó documentado honestamente (R-11, GAP-B) y sujeto a esta ratificación.
>
> **Excepción a la frase anterior — el detalle de lo ratificado:** La slice de F3 sí toca `Shared`: `PagedPayload<T>` (`src/Shared/Infrastructure/Presentation/Results/HttpOkPagedResult.cs`) pasó de `internal` a `public`.
>
> - **Por qué:** `controllers.md` §5.5 y `casos-de-uso.md` §5.5 prescriben declarar el 200 de un listado como `ApiSuccessResponse<PagedPayload<T>>`, pero el tipo era interno a `Shared.Presentation` y ningún controller podía nombrarlo, así que el contrato paginado no se publicaba en OpenAPI. El cambio alinea `Shared` con su propia documentación.
> - **Alcance:** solo la visibilidad de un tipo existente; no se agrega abstracción, no cambia comportamiento, y ningún consumidor actual se rompe. Pero es **superficie compartida** por todos los contextos, empaquetada dentro de una slice de contexto.
> - **Segunda superficie compartida, del mismo esfuerzo:** al resolver `GAP-B` con la opción **B-1** se agregó el puerto `ITenantCodeProvider` a `Shared/Application/Ports/` (un solo miembro, `string? Current`) y `ITenantConnectionInitializer.Initialize` pasó a recibir también el código de tenant. Esta sí es una **abstracción nueva** en `Shared`, no un cambio de visibilidad, y era el costo declarado de B-1 en §9.2. Requiere la misma ratificación.
> - **Quién firma:** `.github/CODEOWNERS` declara `* @dparra`, de modo que la revisión del PR ya queda enrutada a esa persona por CODEOWNERS. **Debe reconocerlo explícitamente antes del merge**, no pasar inadvertido entre los archivos del contexto.
> - **Si se rechaza:** la alternativa es dejar `PagedPayload<T>` interno y declarar el 200 con un tipo propio del contexto que replique `{ items, totalCount }` — se evita tocar `Shared` a cambio de duplicar la forma del contrato en cada contexto que pagine.
...
`id: F3 · depende_de: F2.7 · tarea: (sin asignar) rama Feature/business-status-get-all-useCase · estado: done`
...
- **Punto 3 entregado (2026-08-21), cerrando el paso.** La caché L2 se implementó al resolver `GAP-B` con la opción **B-1** (§9.2): `BusinessStatusRepository` recibe `ICacheStore` e `ITenantCodeProvider`, `GetAsync` hace cache-aside con TTL de 10 minutos sobre `BusinessStatusListSnapshot`, y no cachea nada cuando no hay tenant resuelto. 8 pruebas nuevas en `BusinessStatusRepositoryCacheTests`. **Queda pendiente la ratificación del Tech Lead** por el puerto agregado a `Shared` (§5.5, `DESVIACIÓN-2`).
- **Ejecución inicial (2026-08-21) — puntos 1 y 2, con el 3 diferido por GAP-B.** Quedan hechos el caso de uso, los DTOs, el mapping, el validador, la acción de controller y el registro DI, con 15 pruebas unitarias nuevas. **La caché L2 no se implementó:** ver `⚠️ GAP-B` en §9.2. Dos desviaciones de la lista de archivos de este paso, ambas por convención real del repositorio:
  - El validador vive en `src/Infrastructure/Validation/FluentValidation/BusinessStatuses/GetBusinessStatusesInputValidator.cs`, **no** en `src/Api/Validators/`: `AddContextValidators()` (`src/Infrastructure/Extensions/ValidatorRegistrationExtensions.cs`) escanea **solo el ensamblado Infrastructure**, así que un validador en `Api` no se registraría y `[ValidateRequest]` lo saltaría en silencio. Es además la ubicación que indica `validaciones.md` y la que ya usa `CreateBusinessStatusInputValidator`. Sus pruebas se sumaron a `tests/UnitTests/Infrastructure/Validation/BusinessStatusValidatorsTests.cs`.
  - `PagedPayload<T>` (`Shared/Infrastructure/Presentation/Results/HttpOkPagedResult.cs`) pasó de `internal` a `public`. `controllers.md` §5.5 y `casos-de-uso.md` §5.5 prescriben declarar el 200 como `ApiSuccessResponse<PagedPayload<T>>`, pero el tipo era interno a `Shared.Presentation` y ningún controller podía nombrarlo. Es un cambio de visibilidad, no una abstracción nueva; sin él el contrato paginado no se publica en OpenAPI.
...
`id: F6 · depende_de: F5 · tarea: (sin asignar) rama Feature/business-status-update-useCase · estado: done`
...
- **Ejecución (2026-08-21).** Tres desviaciones respecto de lo escrito arriba, todas registradas:

  1. **Orden.** Se ejecutó F6 con F3 y F4 todavía en `pending`, igual que F5 y por la misma instrucción del desarrollador. El `depende_de: F5` sí se cumplió (F5 está `done`); el `BusinessStatusesController` y su `CacheTag` ya existían y esta slice **reutiliza** ambos, agregando únicamente su acción `UpdateBusinessStatus` — no declara un segundo `CacheTag`.
  2. **`DESVIACIÓN-2` aplicada también acá — ubicación del validador.** Confirmado el error de la lista de archivos de este paso, ya anticipado en la nota de ejecución de F5: el validador **no** va en `src/Api/Validators/UpdateBusinessStatusInputValidator.cs`, sino en `src/Infrastructure/Validation/FluentValidation/BusinessStatuses/UpdateBusinessStatusInputValidator.cs`, porque `ValidatorRegistrationExtensions.AddContextValidators()` escanea solo el ensamblado `Infrastructure`. Verificado en ejecución real, no por inspección: un `PUT` con `name: "   "` y `color: "zzzzzz"` responde `400` con `type: VALIDATION` y los dos `details`, lo que prueba que `[ValidateRequest]` sí encontró el validador. Su test vive en `tests/UnitTests/Infrastructure/Validation/BusinessStatusValidatorsTests.cs`, junto al del alta. `UpdateBusinessStatusInputValidator` reutiliza la constante `CreateBusinessStatusInputValidator.ColorPattern` en vez de repetir la expresión regular.
  3. **La invalidación L2 no se implementó — mismo `GAP-B` (§9.2) que F5, ver R-10.** El caso de uso **no** inyecta `ICacheStore` y **no** llama `RemoveByPrefixAsync`, por la razón ya registrada en F5: el código de tenant no llega a la capa de aplicación, y una invalidación con un prefijo sin `.Tenant(...)` no coincidiría con ninguna llave que escriba F3. La invalidación L1 (`[OutputCacheInvalidate(CacheTag)]`) **sí** quedó puesta. Hoy el impacto es nulo: la caché L2 del listado todavía no existe porque la crea F3.

  Notas de implementación que **no** son desviaciones: el caso de uso sigue el patrón canónico de `docs/plantilla/casos-de-uso.md` §5.2 (cargar → `aggregate.Update(args)` → `repository.Update` → `unitOfWork.CommitAsync`), y solo sella con `Context`/`Origin` el error del paso del dominio — los del repositorio y del Unit of Work se propagan tal cual. La ruta se declara `[HttpPut("{id:int}")]`; la restricción `:int` es la que hace que un id no numérico no llegue al caso de uso. No se agregó validación estructural del `id` de ruta (`GreaterThan(0)`, §6.3): `IStructuralValidator<T>` opera sobre el DTO del cuerpo, no sobre parámetros de ruta, y un id ≤ 0 ya resuelve en `404` por el `GetByIdAsync`. Si se quiere el `400`, es una pieza transversal que corresponde decidir a F4/F7, que comparten el mismo problema.
- **Ejecutado:** `dotnet build Service.slnx` en verde. `dotnet test tests/UnitTests/UnitTests.csproj` → **529/529**, de los cuales **41** son nuevos de esta slice (13 del caso de uso —25 casos contando los `Theory`—, 4 del controller, 24 del validador). Cobertura de línea global **97,7 %** (umbral del gate: 90 %), con los cuatro artefactos nuevos al **100 %** de línea y de rama. Verificación adicional contra la aplicación levantada (`multitenancy` deshabilitada, base en memoria): `PUT /business-statuses/{id}` aparece en `/openapi/v1.json` bajo el tag `business-statuses` con `200/400/404/500`, y responde `200` al reemplazo completo, `400` al mover un intermedio a 100 (`DOMAIN_VALIDATION`, INV-1), `400` a la validación estructural de nombre y color, y `404` a un id inexistente.
...
`id: F7 · depende_de: F6 · tarea: (sin asignar) rama Feature/business-status-delete-useCase · estado: done`
...
- **Detalle:**
  ...
  2. **Endpoint:** `DeleteBusinessStatus([FromRoute] int id)` → `HttpNoContentResult`, con `[ProducesResponseType]` 204/404/409 y `[OutputCacheInvalidate(CacheTag)]`. `AddBusinessStatusServices()` agrega `services.AddScoped<IDeleteBusinessStatusUseCase, DeleteBusinessStatusUseCase>();` — esta era la última de las cinco líneas de registro, una por slice. El controller queda con sus **cinco** endpoints y `AddBusinessStatusServices()` con sus **cinco** casos de uso solo cuando F3 aporte el suyo: hoy son **cuatro** de cada uno (ver la nota de integración más abajo).
...
- **Ejecución (2026-08-21).** Cuatro notas, todas registradas:

  1. **Orden.** Se ejecutó F7 con F3, F4 y F6 en `pending`, por instrucción explícita del desarrollador — misma situación que F5. El `depende_de: F6` queda como dependencia **documental**, no funcional: el borrado solo necesita `EnsureCanBeDeleted` (F1.4), `GetByIdAsync`/`RemoveAsync` (F2.5, F2.6) y el `UnitOfWorkAdapter` de la plantilla, todos ya en verde. La acción se agregó sobre el `BusinessStatusesController` que creó F5, reutilizando su `CacheTag` sin declarar otro; F3 y F4 agregan las suyas sobre el mismo archivo.
     > **Integración con la rama de contexto — dos veces (2026-08-21).** La primera vez, `Feature/Context-Business-Status` ya traía **F6** mergeada (PR #24); la segunda, **F4** (PR #38). Las tres slices tocan los mismos tres archivos, así que ambos merges conflictuaron en `BusinessStatusesController.cs`, `BusinessStatusServiceExtensions.cs` y `BusinessStatusesControllerTests.cs`, y en las dos ocasiones se resolvió **conservando todas las slices**. Estado tras el segundo merge: el controller expone `POST`, `GET /{id}`, `PUT` y `DELETE` —los cuatro endpoints construidos, falta solo el listado de F3— y `AddBusinessStatusServices()` registra los cuatro casos de uso.
     > Tres decisiones de la resolución: (a) el `DELETE` adoptó la restricción de ruta `{id:int}` que ya usaba el `PUT`, por coherencia dentro del archivo (la plantilla escribe `("{id}")` a secas, y el `GET` de F4 quedó así — no se tocó, es de otra slice; la restricción solo evita que un id no numérico llegue al *model binding*); (b) el helper de pruebas quedó en el par `RunAsync` (texto crudo, para el 204 sin cuerpo) + `ExecuteAsync` (JSON), que generaliza el que introdujo F6; (c) F4 trajo `BusinessStatusServiceExtensionsTests`, un teatro de registros DI con **una línea por slice** —el único test que detecta un registro faltante, porque los demás inyectan dobles y nunca pasan por el contenedor—, así que se le agregó el `InlineData` de `IDeleteBusinessStatusUseCase`.
  2. **La invalidación L2 sigue sin implementarse — `GAP-B` (§9.2) y R-10 (§9.1) ahora también cubren F7.** El caso de uso **no** inyecta `ICacheStore` y **no** llama `RemoveByPrefixAsync`, por la misma razón que F5: la llave de §7.2 se particiona por tenant y `TenantInfo.EntityCode` no sale de `TenantMiddleware`. Hoy el impacto sigue siendo nulo porque F3 —la única slice que escribe esa caché— no existe. La invalidación **L1 sí** quedó puesta (`[OutputCacheInvalidate(CacheTag)]`), y el filtro solo evacúa el tag cuando la respuesta es < 400, así que los caminos 404 y 409 no invalidan nada sin necesidad de código extra.
  3. **El `id` de la ruta no lleva validación estructural.** §6.3 declara `GreaterThan(0)` para el `id` de `DELETE /{id}`, pero `ValidateRequestFilter` **omite los tipos simples** (`IsSimpleType`), de modo que un `IStructuralValidator<int>` nunca se invocaría; la plantilla tampoco valida ningún `id` de ruta (`docs/plantilla/controllers.md` §5.3 usa `[FromRoute] Guid id` sin atributos). Consecuencia: un `id` ≤ 0 responde **404**, no 400 — el repositorio no encuentra la fila. Coherente con la lista de archivos de este paso, que no incluye validador. La misma nota aplicará a F4.
  4. **El caso de uso hace dos viajes a la base** (`GetByIdAsync` + `RemoveAsync`), a diferencia del patrón de `casos-de-uso.md` §5.3, que delega en `RemoveAsync` a secas. No es una desviación: INV-3 es una regla del agregado y solo el agregado puede decidir si la fila es terminal, así que hay que cargarlo antes de borrarlo. El `NotFound` del `GetByIdAsync` hace innecesario el del `RemoveAsync` en el camino feliz, pero éste se conserva porque el contrato de `IRootRepository` lo exige y otras rutas podrían llamarlo directo.
- **Ejecutado:** `dotnet build Service.slnx -c Release` en verde, **0 advertencias**; `dotnet test tests/UnitTests/UnitTests.csproj` → **504/504** antes de integrar la rama de contexto y **543/543** después (529 que ya traía el contexto + los **14** nuevos de esta slice: 9 del caso de uso, 5 del controller). Cobertura de línea global **97,41 %** en la medición previa al merge (gate del CI: 90 %), con `DeleteBusinessStatusUseCase` y `BusinessStatusesController` al **100 %**. Se compiló y probó en `Release` porque el `Debug` estaba bloqueado por dos instancias de `Api.exe` en ejecución (sesión de depuración de Visual Studio); mismo código, mismo resultado.
...
| R-10 | **Ninguna de las tres mutaciones invalida la caché L2 del listado.** `CreateBusinessStatusUseCase` (F5), `UpdateBusinessStatusUseCase` (F6) y `DeleteBusinessStatusUseCase` (F7) no llaman `RemoveByPrefixAsync`, porque la llave de §7.2 se particiona por tenant y el código de tenant no llega a la capa de aplicación (ver `GAP-B`) | F5 · F6 · F7 · §7.2 · D12 | Medio **a partir de que F3 exista**: hasta 10 minutos de listado obsoleto tras cada alta, edición o borrado hecho por el servicio. Hoy nulo: sin F3 no hay caché L2 que invalidar | Ninguna en F5, F6 ni F7. Se cierra con `GAP-B`: quien implemente F3 introduce el accesor de código de tenant y agrega la línea de invalidación a las tres. La invalidación L1 (`[OutputCacheInvalidate]`) sí está puesta en las tres |
| R-10 | **Ni el alta ni la edición invalidan la caché L2 del listado.** `CreateBusinessStatusUseCase` (F5) y `UpdateBusinessStatusUseCase` (F6) no llaman `RemoveByPrefixAsync`, porque la llave de §7.2 se particiona por tenant y el código de tenant no llega a la capa de aplicación (ver `GAP-B`) | F5 · F6 · §7.2 · D12 | Medio **ya**: F3 activó la caché L2, así que un alta, edición o borrado deja el listado obsoleto hasta 10 minutos por tenant. Nulo mientras `Cache__L2Enabled` siga en `false`, que es su valor por defecto | **Mitigación vigente, indicada por el Tech Lead el 2026-08-28: no poner `Cache__L2Enabled=true` en ningún ambiente hasta que la invalidación aterrice en F5/F6/F7.** `GAP-B` ya está cerrado, así que no hace falta ningún cambio más de middleware: cada caso de uso de escritura inyecta `ICacheStore` + `ITenantCodeProvider` y, **post-commit**, llama `RemoveByPrefixAsync(CacheKey.For("businessstatus").Tenant(tenant).Prefix("list"))`. Mecánica ya validada: el prefijo `ctx:businessstatus:v1:t:{tenant}:list` con el `SCAN MATCH "…*"` de `RedisCacheStore` borra exactamente las entradas `…:list:{digest}` que escribe `GetAsync`, y las llaves de L2 van crudas (sin el `InstanceName` que sí lleva L1), así que lectura, escritura y barrido coinciden. **Dos requisitos:** (1) la invalidación **debe** leer el mismo `ITenantCodeProvider.Current` que la lectura, o el prefijo no coincide en silencio; (2) hay que guardar el tenant nulo o vacío —y el que contenga `':'`— porque `CacheKey.Tenant` lanza. La invalidación L1 (`[OutputCacheInvalidate]`) sí está puesta en ambas |
| R-9 | ~~**Un porcentaje persistido fuera del rango de `int` lanzaría `OverflowException` en el mapper.**~~ **Cerrado (2026-08-28)** La conversión de F2.3 (`(int)row.Percentage.Value`) toleraba el no-entero y el nulo, pero no un entero que no quepa en `int`: la columna es `decimal(20,5)` y no tiene CHECK que la limite a 0-100 (D1) | F2.3 · D1 · D5 | Bajo: exigiría un dato ≥ 2.147.483.648 en una columna de porcentaje; ningún tenant de la muestra lo tiene (Discovery §4.4) | **Implementado** junto con el cierre del blocker INV-2/INV-3: `ToWholePercentage` rechaza (devuelve `null`) todo valor fuera de `[0, 100]` antes del cast, así que un número más allá de `int` ya no llega a la conversión. Ver R-13 |
| R-11 | **Un cambio de superficie compartida viaja dentro de una slice de contexto.** `PagedPayload<T>` pasó de `internal` a `public` en la slice de F3, contradiciendo la frase de §5.5 «este plan no tiene PR de `Shared`». El riesgo no es el cambio —es correcto y alinea `Shared` con su documentación— sino que se mergee sin que el dueño lo advierta entre los archivos del contexto | `DESVIACIÓN-2` (§5.5) · revisión del desarrollador, 2026-08-21 | Bajo en lo técnico (solo visibilidad, sin cambio de comportamiento); medio en gobierno de la superficie compartida | Registrado como `DESVIACIÓN-2` en §5.5 y señalado en el cuerpo del commit de la slice. `.github/CODEOWNERS` (`* @dparra`) ya enruta la revisión al dueño; **debe reconocerlo explícitamente antes del merge**. Alternativa documentada si se rechaza |
| R-13 | ~~**Una fila terminal con residuo decimal escapaba a INV-2/INV-3.**~~ **Cerrado (2026-08-28)** El mapper de lectura convertía a `null` cualquier porcentaje no entero, así que un «Ganado» legado guardado como `100,00001` se leía como intermedio: `IsTerminal` daba `false`, `EnsureCanBeDeleted` autorizaba el borrado y `ValidatePercentageForUpdate` permitía moverlo a 50 % | Revisión 2026-08-28 · D5 · F2.3 | Era Alto sobre filas terminales legadas con ruido decimal, alcanzable por `DELETE` y `PUT` | **Implementado (enfoque A):** `ToWholePercentage` reconoce como terminal todo valor a ≤ `0.001` de 0 o 100 (`TerminalSnapTolerance`), de modo que el ruido se resuelve hacia el valor reservado y la fila conserva la protección de INV-2/INV-3. Un residuo intermedio (0,4; 50,5; 99,9) sigue leyéndose como `null` (D5 intacto). **Limitación residual:** `GetActiveTerminalsAsync` (F8.1, aún sin consumidor) filtra `x.Percentage == 0/100` a nivel de BD, antes del mapper, así que una fila con residuo no la vería; cuando F8.1 aterrice, su consulta debe redondear o rangear igual |
| R-12 | **El `PUT` no puede expresar «sin porcentaje».** La lectura puede devolver `percentage: null` (D14(d) replica la nulabilidad de `negest_porcentaje`), pero el `PUT` —reemplazo completo— exige el campo (`percentage` es `decimal?` con `NotNull`, la mitigación que evita el 400 engañoso del alta). Editar solo el nombre de una fila legada con porcentaje nulo le asigna un porcentaje: no hay forma de reponer el `null` por contrato | Revisión 2026-08-28 · D5 (escritura: solo enteros en 0<x<100) · D14(d) | Bajo: solo afecta a filas legadas con `negest_porcentaje` nulo, y el cambio es explícito (el cliente **envía** el porcentaje que queda). Ninguna fila creada por el servicio nace con porcentaje nulo | **Aceptado.** La intención de D5 fija que la escritura solo maneja enteros; el `null` es un estado de lectura de datos legados, no un valor asignable. Si el negocio requiere reponer el `null`, es un cambio de contrato (porcentaje anulable en el Args de actualización), no una corrección |

### 9.2 GAPs consolidados

**✅ GAP-B — código de tenant para la llave L2 · abierto y RESUELTO CON B-1 el 2026-08-21 · pendiente de ratificación del Tech Lead por tocar `Shared`**

| | |
|---|---|
| **Resolución** | **B-1**, por instrucción del desarrollador: el código de tenant viaja al objeto de sesión por petición y se lee desde un puerto propio |
| **Qué se construyó** | `ITenantCodeProvider` (`Shared/Application/Ports/`, un solo miembro `string? Current`) · `ITenantConnectionInitializer.Initialize(connectionString, entityCode)` · `TenantContext` implementa los tres contratos y expone `Current` **sin lanzar** (a diferencia de `ConnectionString`) · `TenantMiddleware` pasa `result.Value.EntityCode` · `NoTenantCodeProvider` se registra cuando la multitenencia está apagada, para que el puerto resuelva y la caché simplemente se omita en vez de tumbar cada endpoint que lo inyecta |
| **Pendiente** | **Ratificación del Tech Lead** (`@dparra` por CODEOWNERS): B-1 agrega un puerto a `Shared.Application.Ports`, la segunda superficie compartida que toca este esfuerzo — ver `DESVIACIÓN-2` en §5.5 |
| **Qué desbloquea** | Ya entregado el punto 3 de **F3** (cache-aside del listado). **Queda por hacer** la invalidación post-commit de **F5**, **F6** (ambas ejecutadas sin ella, R-10) y **F7**: el puerto ya existe, así que es agregar `ICacheStore` + `RemoveByPrefixAsync` a esos tres casos de uso |
| **Defecto D-12** | Corregido en la lectura del listado; sigue vivo en la coherencia de las escrituras hasta que F5/F6/F7 invaliden |

La caché L2 que especifican D12 y §7.2 exige la llave `CacheKey.For("businessstatus").Tenant(tenantCode)...`, pero **el servicio no expone en ninguna capa el código de tenant de la petición en curso**:

- `TenantMiddleware` lee `EntityCode` / `X-Entity-Code` y tiene el código en la mano (`result.Value.EntityCode`), pero solo escribe el **connection string** en `TenantContext` a través de `ITenantConnectionInitializer`.
- `IDbConnectionProvider`, el único puerto que Infrastructure puede inyectar para hablar de tenancy, expone **únicamente** `ConnectionString`.
- `IHttpContextAccessor` no está registrado, y `Infrastructure` no tiene `FrameworkReference` a `Microsoft.AspNetCore.App`, así que el repositorio no puede alcanzar el `HttpContext` sin meter ASP.NET en la capa de persistencia.

Tres salidas posibles, en orden de preferencia del ejecutor:

| # | Opción | Costo | Observación |
|---|---|---|---|
| B-1 | Llevar el código de tenant al objeto de sesión por petición: `ITenantConnectionInitializer.Initialize(connectionString, entityCode)` y un puerto de lectura nuevo (p. ej. `ITenantCodeProvider`) implementado por `TenantContext` | Toca `Shared.Application.Ports` (contradice §5.5, «este plan no tiene PR de `Shared`») + `Api` (middleware y sesión) | El middleware ya tiene el dato. Es la única que sirve igual a F3 (lectura) y a F5-F7 (invalidación), y no filtra ASP.NET a Infrastructure |
| B-2 | `IHttpContextAccessor` en el repositorio | `AddHttpContextAccessor()` + `FrameworkReference` en `Infrastructure` | Mete la capa web dentro de persistencia |
| B-3 | Particionar por un hash estable del connection string en vez del código | Ninguno: `IDbConnectionProvider` ya es inyectable | La llave deja de ser legible (`t:{hash}`), se aparta de la forma documentada en §7.2 y complica correlacionar en Redis |

**Forma implementada** en `BusinessStatusRepository.GetAsync` —no en el caso de uso—, con `BusinessStatusListSnapshot` como tipo cacheado:

```csharp
// El repositorio recibe por constructor ICacheStore y el accesor del tenant.
var key = CacheKey.For("businessstatus").Tenant(tenant.Current).Resource("list", StableHash(filter, page));

var cached = await cacheStore.GetAsync<BusinessStatusListSnapshot>(key, cancellationToken).ConfigureAwait(false);
if (cached is not null)
    return PagedResult<BusinessStatusAggregate>.Success([.. cached.Items.Select(ToAggregate)], cached.TotalCount);

// … la consulta actual …
await cacheStore.SetAsync(key, snapshot, TimeSpan.FromMinutes(10), cancellationToken).ConfigureAwait(false);
return PagedResult<BusinessStatusAggregate>.Success(items, totalCount);
```

Se cachea el **snapshot serializable**, nunca el agregado (constructor privado), solo en éxito, y sin cachear cuando no hay tenant resuelto. La llave identifica la combinación filtro+página con un **digest SHA-256** y no con `GetHashCode`, que está aleatorizado por proceso: dos réplicas nunca compartirían entrada y un reinicio huerfanaría todas las llaves. El nombre va prefijado por su longitud para que `("a|b", null)` y `("a", "b")` no colisionen en una misma entrada.

Las pruebas (`BusinessStatusRepositoryCacheTests`, 8 casos) corren contra `JsonRoundTripCacheStore` y no contra un doble, para que el snapshot se (de)serialice de verdad: un tipo que `System.Text.Json` no pueda reconstruir degradaría a 0 % de aciertos en silencio, y así falla la prueba en vez de producción. El caso del hit se prueba **borrando la fila de la base** entre la primera y la segunda llamada: lo que devuelva la segunda no puede venir de la base.

**Ningún otro GAP `BLOQUEANTE` está abierto. Ningún paso de §8 nace `blocked`.**

| GAP | Estado | Resolución del 2026-08-14 |
|---|---|---|
| **GAP-B (código de tenant para la llave L2)** | **Abierto · espera decisión del Tech Lead** (`@dparra`) · detectado 2026-08-21 al ejecutar F5, reincidente en F6 y F3 | Ver el bloque **⚠️ GAP-B** al inicio de esta sección: ahí están el diagnóstico, las tres opciones (B-1 preferida), el dueño de la decisión, la forma objetivo del código y la condición de cierre. **Una sola fuente para este GAP; no duplicar la narrativa aquí** |
...
| 2026-08-21 | Ejecución de **F6** (`PUT /business-statuses/{id}`), también fuera de orden (F3 y F4 siguen en `pending`). Se crean el caso de uso, sus DTOs y mapping, el validador estructural y el registro DI; el `BusinessStatusesController` recibe su segunda acción y **reutiliza** el `CacheTag` que declaró F5. 41 tests unitarios nuevos, suite completa en 529/529, cobertura de línea global 97,7 % con los cuatro artefactos nuevos al 100 %. Tres desviaciones registradas en el paso: orden, `DESVIACIÓN-2` confirmada en ejecución real (el validador vive en `Infrastructure/Validation/FluentValidation/`, no en `Api/Validators/`) y la **invalidación L2 no implementada**, mismo `GAP-B` que F5 → R-10 actualizado para cubrir ambas slices | Ejecución del plan |
| 2026-08-21 | Ejecución de **F7** (`DELETE /business-statuses/{id}`) fuera de orden, con F3 y F4 en `pending`, por instrucción del desarrollador. Se crean `IDeleteBusinessStatusUseCase` y `DeleteBusinessStatusUseCase` (sin DTOs ni mapping: el borrado no los tiene), la tercera acción del controller y el registro DI; 14 tests unitarios nuevos. Cuatro notas registradas en el paso: orden, `GAP-B`/R-10 extendidos a F7 (la invalidación L2 sigue sin implementarse; la L1 sí está), el `id` de ruta sin validación estructural porque `ValidateRequestFilter` omite los tipos simples (un `id` ≤ 0 responde 404, no 400), y los dos viajes a la base que exige INV-3. Al integrar la rama de contexto —que ya traía F6— el conflicto en el controller, el registro DI y sus tests se resolvió **conservando ambas slices**, y el `DELETE` adoptó la restricción de ruta `{id:int}` de la acción hermana | Ejecución del plan |
