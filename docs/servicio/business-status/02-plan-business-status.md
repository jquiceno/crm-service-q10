...
> **⚠️ Excepción a la frase anterior — `DESVIACIÓN-2`, pendiente de sign-off del dueño de `Shared`.** La slice de F3 sí toca `Shared`: `PagedPayload<T>` (`src/Shared/Infrastructure/Presentation/Results/HttpOkPagedResult.cs`) pasó de `internal` a `public`.
>
...
| R-10 | **Ni el alta ni la edición invalidan la caché L2 del listado.** `CreateBusinessStatusUseCase` (F5) y `UpdateBusinessStatusUseCase` (F6) no llaman `RemoveByPrefixAsync`, porque la llave de §7.2 se particiona por tenant y el código de tenant no llega a la capa de aplicación (ver `GAP-B`) | F5 · F6 · §7.2 · D12 | Medio **a partir de que F3 exista**: hasta 10 minutos de listado obsoleto tras cada alta o edición hecha por el servicio. Hoy nulo: sin F3 no hay caché L2 que invalidar | Ninguna en F5 ni en F6. Se cierra con `GAP-B`: quien implemente F3 introduce el accesor de código de tenant y agrega la línea de invalidación a F5, F6 y F7. La invalidación L1 (`[OutputCacheInvalidate]`) sí está puesta en ambas |
