...
### 5.6 `DESVIACIÓN-1` — nombre de la entidad de persistencia
...
- **Desviación:** la entidad de persistencia se llama `BusinessStatusRow`, en `Infrastructure/Persistence/EntityFramework/BusinessStatuses/Entities/`.
- **Alcance:** solo el nombre del tipo de fila. El mapper, la configuración y el repositorio conservan la convención de la plantilla.
- **Pide sign-off del Tech Lead.** Si se rechaza, la alternativa es mantener `BusinessStatus` y calificar cada uso (`Entities.BusinessStatus`) como hace el ejemplo de `contextos.md`.
...
- **Hecho cuando:** el tipo existe con las cinco propiedades y la nulabilidad de la tabla, y el proyecto compila. El nombre `BusinessStatusRow` queda anotado en el PR como `DESVIACIÓN-1` pendiente de sign-off.
...
- **Detalle:** `public DbSet<BusinessStatuses.Entities.BusinessStatusRow> BusinessStatuses => Set<BusinessStatuses.Entities.BusinessStatusRow>();`
...
`id: F5 · depende_de: F4 · tarea: (sin asignar) rama feat/business-status-create · estado: pending`
...
**No queda ningún GAP `BLOQUEANTE` abierto. Ningún paso de §8 nace `blocked`.**
