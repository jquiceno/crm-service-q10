...
- **Ejecución (2026-08-21).** Cuatro notas, todas registradas:
  ...
  1. **Orden.** Se ejecutó F7 con F3, F4 y F6 en `pending`, por instrucción explícita del desarrollador — misma situación que F5. El `depende_de: F6` queda como dependencia **documental**, no funcional: el borrado solo necesita `EnsureCanBeDeleted` (F1.4), `GetByIdAsync`/`RemoveAsync` (F2.5, F2.6) y el `UnitOfWorkAdapter` de la plantilla, todos ya en verde. La acción se agregó sobre el `BusinessStatusesController` que creó F5, reutilizando su `CacheTag` sin declarar otro; F3 y F4 agregan las suyas sobre el mismo archivo.
     > **Integración con la rama de contexto (2026-08-21).** Al actualizar la rama contra `Feature/Context-Business-Status` resultó que **F6 ya estaba mergeada** ahí (PR #24). El conflicto en `BusinessStatusesController.cs`, `BusinessStatusServiceExtensions.cs` y `BusinessStatusesControllerTests.cs` se resolvió **conservando las dos slices**: el controller queda con `POST`, `PUT` y `DELETE`, y el `DELETE` adoptó la restricción de ruta `{id:int}` que ya usaba el `PUT`, por coherencia dentro del mismo archivo (la plantilla escribe `("{id}")` a secas; la restricción solo hace que un id no numérico no llegue ni al *model binding*). El helper de pruebas quedó en el par `RunAsync` (texto crudo, para el 204 sin cuerpo) + `ExecuteAsync` (JSON), que generaliza el que introdujo F6.
