...
* ~~**Orden en `F3.5`:** primero `ExistsAsync`, después `IsUsedAsync`.~~ **Retirado el 2026-09-04**: el borrado ya no comprueba existencia, es idempotente, y `IsUsedAsync` queda de primera. El scan de ~300.000 filas de `neg_cau_consecutivo` (R2) lo paga ahora **todo** `DELETE`; sigue aceptado, y el arreglo, si hace falta, es el índice.
