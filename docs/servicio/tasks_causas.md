...
| Fecha | Cambio |
|---|---|
| 2026-09-04 | **Segunda ronda de revisión del PR de T11 (#48), la de los cinco comentarios que se habían quedado sin ver.** Con `gh` autenticado se auditaron los 13 hilos del PR contra los 11 commits: 8 ya estaban aplicados, 5 faltaban, y **ninguno estaba marcado como resuelto en GitHub**, así que el estado de la UI no sirve de indicador — el cruce se hizo contra el código. Aplicados cuatro: fuera el comentario que justificaba la duración del caché, la duración sube a **3 días** (`3 * 24 * 60 * 60`), y `IdInputDto`/`IdInputValidator` se renombran a **`ConsecutiveIdInputDto`/`ConsecutiveIdInputValidator`** (§3) — la revisión escribió `Sequence…` y el tech lead lo fijó en `Consecutive…`, por el `cau_consecutivoP` del legado. **El quinto —quitar `VaryByQueryKeys`— no se aplicó** y se contestó en el hilo: la política base hace `SetVaryByQuery("EntityCode")`, así que sin claves en el atributo el listado sirve el resultado de un filtro para otro, el fallo ya reproducido en esta rama, ahora con ventana de 3 días. Suite: **466 en verde** |
