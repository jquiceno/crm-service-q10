...
| T5 | **Juan Camilo** | Loss reason usage reader | `F2.6` | `feat/loss-reasons-usage-reader` | `feat/loss-reasons` | 3 | T3 | F5 | ✅ **ejecutada y aprobada** — dos rondas de revisión aplicadas (`0492989`, `790490d`); pendiente de merge |
...
**El backlog está en ejecución. Seis tareas dentro de la base** —T2, T3, T4 (PR #5), T6 (PR #19), T7 (PR #36) y T8 (PR #20)— **con la base verde**. La base recibió además un **merge de `main`** (`f3a8590`), que trae la plantilla al día.

- **Ola 3:** solo queda **T5** (Juan Camilo, `feat/loss-reasons-usage-reader`) — **PR aprobado**, con las dos rondas de revisión aplicadas, pendiente de merge. No bloquea a nadie salvo a **T10**.
- **Ola 4:** T6, T7 y T8 ya están dentro. Queda **T9** (Brayan), **en revisión de PR** (#28) con T8 ya mezclada en su rama, y **T10**, que espera además a T5.
