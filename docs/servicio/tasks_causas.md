...
| T8 | **Brayan** | Create loss reason use case | `F3.3`, `F3.8` | `feat/loss-reasons-create` | `feat/loss-reasons` | 3 | T4 | F3 | 🔄 **en curso** — `F3.3` y `F3.8` `done` y verificados; tres rondas de revisión aplicadas (captura del agregado en los tests, `IsActive` obligatorio, `Name` en el validador estructural); **pendiente de merge** a la base |
| T9 | **Brayan** | Update loss reason use case | `F3.4`, `F3.9` | `feat/loss-reasons-update` | `feat/loss-reasons` | 3 | T4 | F4 | 🔄 **en curso** — `F3.4` y `F3.9` `done` y verificados; `feat/loss-reasons-create` mezclada en la rama para poner el PR al día (build limpio, 409 tests en verde); **pendiente de merge** a la base |
...
- **Ola 4, ya abierta** porque T4 entró: **T8** y **T9** (Brayan) están **terminadas y verificadas en sus ramas** (`feat/loss-reasons-create` y `feat/loss-reasons-update`), pendientes de PR y merge; T6 ya está dentro. Queda **T7** (Juan Camilo), que puede arrancar hoy, y **T10**, que espera además a T5.
...
| Fecha | Cambio |
|---|---|
| 2026-08-28 | **T8 revisada en tres rondas y mezclada en la rama de T9.** Sobre `feat/loss-reasons-create`: los tests del caso de uso pasan a **capturar el agregado** que recibe el repositorio (`Arg.Do` más un stub que devuelve la fila con el `IDENTITY` asignado), porque el stub de valores fijos hacía que las asserts del caso feliz fueran tautológicas y un mapeo que ignorara `IsActive` seguía en verde; `IsActive` pasa de `bool` con default `true` a **`bool?` obligatorio**, con el error de dominio `IsActiveRequired` y validación en las dos capas de `validaciones.md`; y `Name` entra al validador estructural con `.WithState()`, que es lo que `FluentRequestValidationAdapter` lee para no perder el atributo `max` del catálogo. Es un **cambio de contrato**: `isActive` ya no tiene default y su `Description` dice `Required`. Después se mezcla `feat/loss-reasons-create` en `feat/loss-reasons-update`: único conflicto en esta tabla —cada rama marcó su propia fila y dejó la otra en ⬜—, resuelto conservando las dos; build limpio y **409 tests en verde**. Ambas siguen **pendientes de merge a la base** |
