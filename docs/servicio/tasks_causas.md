...
| T5 | **Juan Camilo** | Loss reason usage reader | `F2.6` | `feat/loss-reasons-usage-reader` | `feat/loss-reasons` | 3 | T3 | F5 | ✅ **mergeada** a la base — PR #18, merge `06e6d84`; dos rondas de revisión aplicadas (`0492989`, `790490d`) |
...
| T7 | **Juan Camilo** | Get loss reason by id use case | `F3.2`, `F3.7` | `feat/loss-reasons-get-by-id` | `feat/loss-reasons` | 2 | T4 | F2 | ✅ **mergeada** a la base — PR #36, merge `55106ea` |
...
| T10 | **Juan Camilo** | Delete loss reason use case | `F3.5`, `F3.10` | `feat/loss-reasons-delete` | `feat/loss-reasons` | 3 | T4, T5 | F5 | ✅ **ejecutada** — `F3.5` y `F3.10` `done` y verificados; una ronda de revisión aplicada. **Enmendada el 2026-09-04** en `fix/loss-reasons-delete-drop-exists-check`: el borrado deja de comprobar existencia y pasa a ser idempotente (204 en un id inexistente), 5 tests en vez de 6 |
...
La única regla que traía esta franja **ya no aplica**:
* ~~**Orden en `F3.5`:** primero `ExistsAsync`, después `IsUsedAsync`.~~ **Retirada el 2026-09-04**: el borrado ya no comprueba existencia, es idempotente, y `IsUsedAsync` queda de primera. La verificación que dejó el merge de T10 —«el orden es el correcto»— vale para el código de entonces, no para el de ahora. El scan de ~300.000 filas de `neg_cau_consecutivo` (R2) lo paga desde ya **todo** `DELETE`; sigue aceptado, y el arreglo, si hace falta, es el índice.
...
| 2026-08-28 | **T7 ejecutada** (`F3.2`, `F3.7`, Juan Camilo — rama `feat/loss-reasons-get-by-id`): la consulta por id con sus 4 archivos coubicados y sus 3 tests en verde (388 en la suite). **Son 4 archivos, no 5**: no hay `InputDto` porque la entrada es un `int`. El caso de uso inyecta solo `ILossReasonRepository` y **no declara `Origin`** —no origina errores, propaga el del repositorio—, que es lo que hace que el 404 salga del `ErrorType` y no de un `if` en el controller. Se aplican desde el arranque las dos reglas que salieron de la revisión de T6: `Description` de los DTOs **en inglés** y `{X}Mapping.cs` solo con `ToOutputDto()`. La rama se sacó de un punto viejo de la base y se rebasó con un merge de `feat/loss-reasons` (sin conflictos); con eso la `ProjectReference` a `LossReason.Application` **la aporta T6**, así que T7 no toca `UnitTests.csproj`, como anticipaba §3 |
| 2026-08-28 | **T8 mergeada a la base** (PR #20, merge `56f1eaa`) y **T9 en revisión de PR** (#28). Antes del merge, T8 pasó por tres rondas de revisión: los tests del caso de uso pasan a **capturar el agregado** que recibe el repositorio (`Arg.Do` más un stub que devuelve la fila con el `IDENTITY` asignado), porque el stub de valores fijos hacía tautológicas las asserts del caso feliz —un mapeo que ignorara `IsActive` seguía en verde—; `IsActive` pasa de `bool` con default `true` a **`bool?` obligatorio**, con el error de dominio `IsActiveRequired` y validación en las dos capas de `validaciones.md`; y `Name` entra al validador estructural con `.WithState()`, que es lo que `FluentRequestValidationAdapter` lee para conservar el atributo `max` del catálogo. Es un **cambio de contrato**: `isActive` ya no tiene default y su `Description` dice `Required`. La base queda con **405 tests en verde**. La rama de T9 ya trae T8 mezclada y suma 409; la revisión debería mirar dos pendientes suyos: las `Description` de `UpdateLossReasonInputDto` siguen en **español** (regla de §3.1, que salió de la revisión de T6) y su `IsActive` sigue siendo `bool` no nulable, asimétrico con el create |
| 2026-08-21 | **T8 ejecutada** (`F3.3`, `F3.8`, Brayan): la creación con sus cinco archivos coubicados y sus 3 tests en verde (388 en la suite). **No inyecta `IUnitOfWorkPort`** — el `IDENTITY` obliga a que `CreateAsync` confirme el insert dentro del repositorio (D3). No toca ningún archivo compartido de §3: la `ProjectReference` que T6 dejó puesta ya alcanza. Commit `a7b0133` en `feat/loss-reasons-create`, **pendiente de merge a la base**. Dos correcciones posteriores de revisión: las `Description` de los DTOs pasan a **inglés** (la regla que salió de la revisión de T6, que se me había escapado), y el assert de D3 de `F3.8` **se reescribe porque no podía fallar** —`DidNotReceive()` sobre un sustituto que nadie inyectaba— y pasa a leer los parámetros del constructor; es una **desviación de la letra de F3.8**, declarada en §9.3 del plan |
...
| 2026-09-04 | **Enmienda al borrado de T10**, en la rama `fix/loss-reasons-delete-drop-exists-check` sobre la base: el `DELETE` deja de comprobar existencia. Fuera el `ExistsAsync` del caso de uso y el `FirstOrDefaultAsync` → `NotFound` de `LossReasonRepository.RemoveAsync`, que pasa a emitir un único `DELETE` con `ExecuteDeleteAsync`; cero filas afectadas es éxito, así que un id inexistente responde **204, no 404**. La validación de uso (`IsUsedAsync` → 409) **no se toca**. Efectos: la regla de §2.2 que exigía existencia-primero **queda retirada** y R2 se agrava —todo `DELETE` paga el scan—; el proveedor in-memory no implementa `ExecuteDelete`, así que los dos tests de comportamiento de `RemoveAsync` salen de `F2.9` y su cobertura pasa a `F5.1`; `F3.10` queda en 5 tests, con 100 % de líneas y ramas. Verificado: build Release (0 errores) y 442/442 en unitarias. El detalle y las consecuencias están en el registro de enmiendas de `workplan_causas.md` |
