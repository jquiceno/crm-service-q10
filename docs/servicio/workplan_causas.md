...
| Param | Tipo | Obligatorio | Default | Validación | Capa |
|---|---|---|---|---|---|
| `id` | `int` | **Sí** | — | `> 0` | FluentValidation (`IdInputValidator`, vía `IdInputDto`) |
...
`GET` → 200 / **400** / 404. `DELETE` → **204 sin cuerpo** / **400** / 404 / 409 (D7). Las dos actions **sí** llevan `[ValidateRequest]`: el `id` viaja envuelto en `IdInputDto` precisamente para que la validación estructural lo alcance.
...
- Verificar: `dotnet build Service.slnx -c Release` — **ejecutado el 2026-08-28: 0 errores, 0 advertencias**
- Nota de revisión (2026-08-28): **los `id` de ruta se validan, y no con el mecanismo obvio.** `ValidateRequestFilter` descarta los tipos simples, así que un validador sobre un `int` nunca correría; una restricción de ruta (`{id:int:min(1)}`) respondería **404**, escondiendo un id mal formado como recurso inexistente. El `id` viaja envuelto en **`IdInputDto`** (`src/Shared/Application/Dtos/`, junto a `PageQueryInputDto`) con su **`IdInputValidator`** (`src/Infrastructure/Validation/FluentValidation/Shared/`), y las tres actions con id ganan `[ValidateRequest]`. Los dos archivos son **adiciones transversales, no del contexto**, y su mensaje es genérico (`Id must be greater than 0.`) porque el validador compartido no sabe de qué contexto viene. Verificado que el enlace de un tipo complejo desde la ruta entrega el valor real y que **OpenAPI sigue documentando `Id[path]`**, no un `route.Id` ni un body.
...
- **Enmienda de la revisión (2026-08-28): no hay política nombrada. `OutputCacheExtensions.cs` no se toca.** La primera entrega registró `"loss-reasons-list"` como el paso pedía; la revisión lo rechazó y mandó guiarse de la documentación. `cache.md` distingue dos mecanismos en su tabla de `[OutputCache]`: `PolicyName` «selecciona una política nombrada **en lugar de** la base», mientras que las demás propiedades del atributo **complementan** la base. Con la segunda vía, la política base sigue viva y con ella el `SetVaryByHeader("X-Entity-Code", "Accept-Language")` que aísla los tenants, sin repetirlo. Lo que queda es un atributo en el controller:
  ...
  La revisión pidió además **`Duration` explícito**: `60` segundos, declarado como constante y compartido con el `GET /{id}` para que las dos lecturas no se desalineen. Es el valor de los dos ejemplos de lectura de `controllers.md`, y encaja con el caso: catálogo administrativo que cambia poco, y toda escritura invalida el tag, así que una lectura rancia solo dura hasta la siguiente mutación. **`EntityCode` se mantiene en la lista de claves**: `VaryByQueryKeys` *restringe* qué claves participan, así que si la lista del atributo pisara la de la base, el tenant desaparecería de la clave.
...
- Verificar: `dotnet test tests/UnitTests -c Release` — **ejecutado el 2026-08-28: 466/466 en verde**, cobertura 97,5 % líneas / 92,5 % ramas. Son **45 tests nuevos** tras la ronda de revisión: 11 del controller (incluido el de la clave de caché), 8 del validador de actualizar, 10 del de listar, 7 del `IdInputValidator`, 5 del contrato de longitud y 4 en `LossReasonAggregateTests` y el validador de crear por el `IsActive` obligatorio del `PUT`.
