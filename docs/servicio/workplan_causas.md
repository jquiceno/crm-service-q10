...
| Param | Tipo | Obligatorio | Default | Validación | Capa |
|---|---|---|---|---|---|
| `id` | `int` | **Sí** | — | `> 0` | FluentValidation (`SequenceIdInputValidator`, vía `SequenceIdInputDto`) |
...
`GET` → 200 / **400** / 404. `DELETE` → **204 sin cuerpo** / **400** / 404 / 409 (D7). Las dos actions **sí** llevan `[ValidateRequest]`: el `id` viaja envuelto en `SequenceIdInputDto` precisamente para que la validación estructural lo alcance.
...
- Verificar: `dotnet build Service.slnx -c Release` — **ejecutado el 2026-08-28: 0 errores, 0 advertencias**
- Nota de revisión (2026-08-28): **los `id` de ruta se validan, y no con el mecanismo obvio.** `ValidateRequestFilter` descarta los tipos simples, así que un validador sobre un `int` nunca correría; una restricción de ruta (`{id:int:min(1)}`) respondería **404**, escondiendo un id mal formado como recurso inexistente. El `id` viaja envuelto en **`SequenceIdInputDto`** (`src/Shared/Application/Dtos/`, junto a `PageQueryInputDto`) con su **`SequenceIdInputValidator`** (`src/Infrastructure/Validation/FluentValidation/Shared/`), y las tres actions con id ganan `[ValidateRequest]`. Los nombres son los que pidió la revisión en sus hilos sobre estos dos archivos. Hubo un desvío intermedio a `Consecutive…`, por el `cau_consecutivoP` del legado, revertido el 2026-09-04. Los dos archivos son **adiciones transversales, no del contexto**, y su mensaje es genérico (`Id must be greater than 0.`) porque el validador compartido no sabe de qué contexto viene. Verificado que el enlace de un tipo complejo desde la ruta entrega el valor real y que **OpenAPI sigue documentando `Id[path]`**, no un `route.Id` ni un body.
...
- Verificar: `dotnet test tests/UnitTests -c Release` — **ejecutado el 2026-08-28: 466/466 en verde**, cobertura 97,5 % líneas / 92,5 % ramas. Son **45 tests nuevos** tras la ronda de revisión: 11 del controller (incluido el de la clave de caché), 8 del validador de actualizar, 10 del de listar, 7 del `SequenceIdInputValidator`, 5 del contrato de longitud y 4 en `LossReasonAggregateTests` y el validador de crear por el `IsActive` obligatorio del `PUT`.
