...
| 2026-09-04 | **Segunda ronda de revisión sobre el PR de T11**, la de los cinco comentarios del 28-ago 21:27–21:44 que no se habían visto. Cuatro aplicados: **(a)** fuera el comentario que justificaba la duración del caché; **(b)** la duración sube de 60 segundos a **3 días**, escrita `3 * 24 * 60 * 60` para que el número siga siendo legible sin comentario; **(c)** y **(d)** `IdInputDto` → **`ConsecutiveIdInputDto`** y `IdInputValidator` → **`ConsecutiveIdInputValidator`**, con sus archivos y su test renombrados. La revisión escribió `Sequence…`; el tech lead lo fijó en **`Consecutive…`**, por el `cau_consecutivoP` del legado. El quinto —quitar `VaryByQueryKeys`— se rechazó primero por error; ver la fila siguiente | **D10** (duración) | §6.2 · §6.3 · §6.4 · §7.3 · §5 (inventario) · `F4.2` · `F4.3` · `F4.4` · §9.1 R8 | ninguna — se corrige en la rama de T11 |
