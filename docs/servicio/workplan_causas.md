...
| `DeleteLossReasonUseCase` | `DELETE /loss-reasons/{id}` | `Task<Result> ExecuteAsync(int id, CancellationToken)` | `IsUsedAsync` → `RemoveAsync` → `CommitAsync`. **Sin `ExistsAsync`**: el borrado es idempotente (enmienda del 2026-09-04) |
...
`GET` → 200 / **400** / 404. `DELETE` → **204 sin cuerpo** / **400** / 409 (D7) — **sin 404**: el borrado es idempotente desde la enmienda del 2026-09-04, así que un id inexistente también responde 204. Las dos actions **sí** llevan `[ValidateRequest]`: el `id` viaja envuelto en `ConsecutiveIdInputDto` precisamente para que la validación estructural lo alcance.
...
| `LossReasonErrors.NotFound(id)` | `NotFound` | 404 | `GET`/`PUT` sobre un id inexistente. **No el `DELETE`**: es idempotente desde la enmienda del 2026-09-04 |
| `LossReasonErrors.InUse(id)` | `Conflict` | 409 | `DELETE` de una causa asignada a un negocio (D7) |
| 547 vía `SqlServerErrorClassifier` | `Conflict` | 409 | carrera perdida en el `DELETE`. Desde la enmienda del 2026-09-04 lo clasifica **`LossReasonRepository.RemoveAsync`**, no el `UnitOfWorkAdapter`: el `ExecuteDeleteAsync` no pasa por el commit, así que la `SqlException` sale ahí |
...
- Hecho cuando: los 21 tests pasan y `LossReasonRepository` queda sin líneas descubiertas en el reporte de cobertura, salvo el `DELETE` de `RemoveAsync` (ver arriba).
...
- Detalle: `Task<Result> ExecuteAsync(int id, CancellationToken)`. Orden obligatorio: `usageReader.IsUsedAsync(id)` → si está en uso, `LossReasonErrors.InUse(id)`; luego `RemoveAsync` + `CommitAsync`. Sin DTOs (204 sin cuerpo).
- **El borrado no comprueba existencia** (enmienda del 2026-09-04): ni aquí ni en el repositorio. Un id que no está borra cero filas y responde igual 204, así que el caso de uso **nunca devuelve `NotFound`**. Consecuencia asumida: **todo** `DELETE` paga ahora el scan del Reader, incluidos los ids inexistentes que antes cortaban en el `ExistsAsync` — ver R2 en §9.1.
- Hecho cuando: borrar una causa en uso devuelve `ErrorType.Conflict` sin llegar a `RemoveAsync`.
- Verificar: `dotnet build Service.slnx -c Release` — **ejecutado el 2026-08-28: exit code 0, 0 errores**
- Nota de ejecución: **sí declara `Origin`**, a diferencia de `F3.2`, porque este caso de uso **sí origina un error propio** (`InUse` cuando está asignada) y lo sella con `with { Origin = Origin }`. Los ajenos —del repositorio y del Reader— los propaga sin tocar. **No sella `Context`**: en todo el servicio, `Context` solo se pone sobre errores de validación del agregado (`F3.3`), mientras que cada `NotFound` se sella únicamente con `Origin`, igual que en `LossReasonRepository`.
...
- Detalle: NSubstitute para repositorio, `ILossReasonUsageReader` e `IUnitOfWorkPort`. **Del Reader hay que cubrir las tres ramas: en uso, libre, y el Reader falla.** Además: en uso → `Conflict` sin llamar a `RemoveAsync`; y un id que no está → **éxito**, con el assert de que `ExistsAsync` **no se llama nunca** (enmienda del 2026-09-04).
- Hecho cuando: los 5 casos pasan y existe el assert de que el caso de uso no consulta la existencia.
- Verificar: `dotnet test tests/UnitTests -c Release` — **ejecutado el 2026-08-28: 6/6 del caso de uso y 417/417 en la suite**
- Nota de ejecución: **son 5 tests.** Fueron 6 mientras existía el `ExistsAsync`: el sexto cubría la rama de `RemoveAsync` fallando, que sin él quedaba en 93,3 % de líneas. La enmienda del 2026-09-04 borra los dos tests de existencia (`ExecuteAsync_WithNonExistentId_ReturnsNotFoundWithoutQueryingTheReader` y `ExecuteAsync_WhenTheRepositoryFails_KeepsTheRepositoryOrigin`, que apuntaba el fallo al `ExistsAsync`) y entra `ExecuteAsync_WithAnIdThatIsNotThere_SucceedsWithoutCheckingExistence`; el fallo del repositorio pasa a `ExecuteAsync_WhenRemoveFails_DoesNotCommitAndKeepsTheRepositoryOrigin`, que fija además la regla de que un borrado que falló **no llega al commit**. Con esos cinco, el caso de uso sigue en **100 % de líneas y de ramas**. Los errores del repositorio y del Reader se arman con `DomainError` de `Shared`, no con `PersistenceErrors` de Infrastructure: es un test de la capa de aplicación, igual que en `F3.6` y `F3.7`.
...
- Nota de ejecución: los 5 verbos verificados en caliente contra la app corriendo (`dotnet run`, multitenencia apagada, EF InMemory): `GET` lista 200 con `{items, totalCount}`, `GET/{id}` 200 y 404, `POST` 201 con el `Id` asignado, `PUT` 200 y 404, `DELETE` 204 sin cuerpo y 404 en el segundo intento —**ese 404 ya no ocurre**: la enmienda del 2026-09-04 hace el borrado idempotente y el segundo intento responde 204; la verificación queda como registro de lo que se probó entonces—. El listado devuelve cuerpos distintos antes y después de un `POST`, lo que prueba que `[OutputCacheInvalidate]` evita servir el listado rancio. Rutas verificadas sin prefijo: `LossReasonsController` → `/loss-reasons` por el `KebabCaseParameterTransformer`, sin escribir la ruta a mano.
...
| R2 | **`neg_cau_consecutivo` no está indexado** (verificado: 6 índices en `tbl_opo_negocios`, ninguno lo cubre) sobre ~300.000 filas. El chequeo de uso del borrado es un scan completo | Aceptado — borrar una causa es una acción administrativa rara. **Agravado el 2026-09-04**: al quitarse el `ExistsAsync` de F3.5, los ids inexistentes ya no cortan antes del Reader, así que **todo** `DELETE` paga el scan. Sigue aceptado por la misma razón; si el volumen de `DELETE` deja de ser marginal, el arreglo es el índice sobre `neg_cau_consecutivo`, no reponer el chequeo |
...
| 2026-09-04 | **El borrado deja de comprobar existencia: pasa a ser idempotente.** Se quita el `repository.ExistsAsync(id)` del `DeleteLossReasonUseCase` y el `FirstOrDefaultAsync` → `NotFound` de `LossReasonRepository.RemoveAsync`, que ahora emite un único `DELETE` con **`ExecuteDeleteAsync`**: cero filas afectadas es éxito, así que un id que no está responde **204, no 404**, y el caso de uso ya no origina `NotFound`. La validación de uso (`IsUsedAsync` → 409) **no se toca** y queda de primera. Tres consecuencias declaradas: (a) **R2 se agrava** — sin el corte por existencia, todo `DELETE` paga el scan de ~300.000 filas, no solo los que existen; (b) `ExecuteDeleteAsync` **se salta el change tracker**, así que el `DELETE` se aplica al instante y el `CommitAsync` posterior queda como no-op — el borrado ya no es atómico con nada más de la misma request, y se conserva la llamada solo para no cambiar el contrato del caso de uso; (c) el proveedor in-memory **no implementa `ExecuteDelete`**, así que los dos tests de comportamiento de `RemoveAsync` en `F2.9` se retiran y su cobertura se traslada a `F5.1` sobre SQL Server real. `RepositoryBaseEF.RemoveAsync` **no se toca**: es infraestructura compartida del template y `LossReasonRepository` no hereda de ella. De paso se corrige el conteo de `F2.9`, que decía 24 tests y eran 23 desde la enmienda de `D6`. Verificado: `dotnet build Service.slnx -c Release` (0 errores, 0 advertencias) y `dotnet test tests/UnitTests -c Release` (442/442 en verde) | **D7 (parcial: se retira el orden existencia-primero, se mantiene el 409 por uso)** | §5.6 (tabla de casos de uso) · `F2.9` · `F3.5` · `F3.10` · `F5.1` · §9.1 R2 | `F3.5` y `F3.10` ya estaban `done` — se corrigen en la rama `fix/loss-reasons-delete-drop-exists-check`; `F5.1` sigue `pending` y absorbe el escenario nuevo |
| 2026-09-04 | **El merge de T11 sobre la enmienda anterior deja dos arreglos.** Al entrar la API (`F4.2`) el borrado ya tiene endpoint, y con él aparecen dos cosas que el merge textual no ve: (a) **la carrera perdida volvía 500 en vez de 409.** Antes, el `DELETE` se confirmaba en el `UnitOfWorkAdapter`, que clasifica el 547 con `SqlServerErrorClassifier`; con `ExecuteDeleteAsync` la `SqlException` sale dentro de `RemoveAsync` y caía en su `catch` genérico. Se añade un `catch (SqlException)` que la clasifica con el overload que ya existía, y §6.5 recupera su 409 —ahora clasificado en el repositorio, no en el commit—; (b) **el `DELETE` declaraba un 404 inalcanzable.** Se retira `[ProducesResponseType(404)]` de la action y su `[EndpointDescription]` dice que el borrado es idempotente; §6.4 y §6.5 se corrigen igual. La rama del 547 **no tiene test unitario**: hace falta SQL Server real, así que la cubre el escenario de 409 de `F5.1`. Verificado: `dotnet build Service.slnx -c Release` (0 errores) y `dotnet test tests/UnitTests -c Release` (490/490 en verde) | — | §6.4 · §6.5 · `F4.2` (nota de ejecución) | ninguna — se resuelve dentro del merge, en `fix/loss-reasons-delete-drop-exists-check` |
