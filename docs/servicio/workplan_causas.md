...
|---|---|---|
| `GET /loss-reasons` | `[OutputCache(Duration = CacheDurationSeconds, Tags = [CacheTag], VaryByQueryKeys = [...])]` | Listado **filtrado**: tiene que variar por `search`, `isActive`, `pageIndex` y `pageSize` además de por tenant. Con solo la política base serviría el resultado de un filtro para otro |
...
`CacheDurationSeconds` son **3 días** (`3 * 24 * 60 * 60`), por indicación de la revisión del 2026-09-04, compartidos por las dos lecturas. Es un catálogo administrativo y toda escritura invalida el tag, así que una lectura rancia vive hasta la siguiente mutación **de ese tenant**.
...
- **Punto abierto de la revisión (2026-09-04): la revisión pide quitar `VaryByQueryKeys`** («esto no es necesario… es debido a una falla en doc»). **No se aplicó**, y el hilo quedó contestado en el PR con la evidencia. Motivo: la política base hace `SetVaryByQuery("EntityCode")`, así que sin claves en el atributo la clave del listado queda **solo** en `EntityCode` y dos filtros distintos comparten entrada — que es literalmente el fallo reproducido dos párrafos arriba, ahora con una ventana de 3 días en vez de 60 segundos. Si la molestia real es que OpenAPI documente mal el endpoint, el arreglo va en la generación del documento, no en la clave de caché. **Pendiente de que la revisión confirme o rectifique**; si insiste, el cambio se aplica y este riesgo pasa a R8 con la mitigación perdida.
...
| 2026-09-04 | **Segunda ronda de revisión sobre el PR de T11**, la de los cinco comentarios del 28-ago 21:27–21:44 que no se habían visto. Cuatro aplicados: **(a)** fuera el comentario que justificaba la duración del caché; **(b)** la duración sube de 60 segundos a **3 días**, escrita `3 * 24 * 60 * 60` para que el número siga siendo legible sin comentario; **(c)** y **(d)** `IdInputDto` → **`ConsecutiveIdInputDto`** y `IdInputValidator` → **`ConsecutiveIdInputValidator`**, con sus archivos y su test renombrados. La revisión escribió `Sequence…`; el tech lead lo fijó en **`Consecutive…`**, por el `cau_consecutivoP` del legado. **El quinto no se aplicó:** quitar `VaryByQueryKeys` deja la clave del listado solo en `EntityCode` —la política base hace `SetVaryByQuery("EntityCode")`— y devuelve el fallo ya reproducido en esta misma rama, ahora con ventana de 3 días; se contestó en el hilo con la evidencia y se espera confirmación | **D10** (duración) | §6.2 · §6.3 · §6.4 · §7.3 · §5 (inventario) · `F4.2` · `F4.3` · `F4.4` · §9.1 R8 | ninguna — se corrige en la rama de T11 |
