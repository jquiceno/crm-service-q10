...
{
    private readonly IGetServiceInfoUseCase _getServiceInfoUseCase = Substitute.For<IGetServiceInfoUseCase>();
    ...
    public async Task GetInfo_WhenUseCaseSucceeds_ReturnsOkWithServiceInfo()
    {
        ...
        _getServiceInfoUseCase.ExecuteAsync(Arg.Any<CancellationToken>())
            .Returns(Result<GetServiceInfoOutputDto>.Success(output));

        var result = await new ServiceInfoController(_getServiceInfoUseCase).GetInfo(CancellationToken.None);
        ...
        await _getServiceInfoUseCase.Received(1).ExecuteAsync(Arg.Any<CancellationToken>());
    }
